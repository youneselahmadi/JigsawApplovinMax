...
public class CUtils
{
    ...
    public static void PrivacyPolicy(string url)
    {
    ...
    public static IEnumerator LoadPicture(string url, string folder, string fileName, Action<Texture2D> callback, Vector2Int? textureSize = null, bool forceUpdate = false, bool cached = true)
    {
        ...
        if (!loaded)
        {
            ...
            if (www.isDone && string.IsNullOrEmpty(www.error))
            {
                ...
                if (textureSize == null || textureSize.Value.x == ((DownloadHandlerTexture)www.downloadHandler).texture.width && textureSize.Value.y == ((DownloadHandlerTexture)www.downloadHandler).texture.height)
                {
                    if (cached)
                    {
               ...
               // var texture = new Texture2D(0, 0, TextureFormat.RGBA32, false);
                ...
                if (callback != null) callback(downloadedTexture);
            }
            else
            {
